...
/**
 * Utility to get target url since TYPO3 6.0
 */
class Tx_MfcResponsivePreview_Utility_Since60 {
	/**
	 * @var array
	 */
	protected $pageRow;
	...
	/**
	 * @param $pageRow
	 * @return Tx_MfcResponsivePreview_Utility_Since60
	 */
	...
	public function getTargetUrl() {
		...
		if ($domainName) {
			...
			if ($page['url_scheme'] == 2 || $page['url_scheme'] == 0 &&
	...
	protected function getAdminCommand() {
		...
		if (is_array($this->pageRow)) {
			$addCommand = '&ADMCMD_view=1&ADMCMD_editIcons=1' . \TYPO3\CMS\Backend\Utility\BackendUtility::ADMCMD_previewCmds($this->pageRow);
		}
		return $addCommand;
	}

	/**
	 * With page TS config it is possible to force a specific type id via
	 * mod.web_view.type for a page id or a page tree. The method checks if a
	 * type is set for the given id and returns the additional GET string.
	 *
	...
	protected function getDomainName($pageId) {
		$domain = \TYPO3\CMS\Backend\Utility\BackendUtility::firstDomainRecord(
